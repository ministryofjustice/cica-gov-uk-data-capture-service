...
describe('replace-json-pointer', () => {
    ...
    describe('JSON pointer that maps to a value with special characters', () => {
        it('should return a string with escaped special characters', () => {
            const content = 'foo ||/foo/bar|| baz';
            const result = replaceJsonPointer(content, {
                foo: {
                    bar: 'b\\ar'
            ...
            expect(result).toEqual('foo b\\\\ar baz');
        });
    });
});
