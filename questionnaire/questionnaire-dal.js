...
function questionnaireDAL(spec) {
    ...
    async function getMetadataByOwner() {
        ...
        try {
            result = await db.query(
                `SELECT id, created, modified, expires, submission_status, questionnaire ->> 'version' AS "questionnaire_version" FROM questionnaire WHERE questionnaire -> 'answers' -> 'owner' ->> 'owner-id' = $1`,
                [ownerId]
            );
        } catch (err) {
            throw err;
    ...
    async function getQuestionnaireMetadataByOwner(questionnaireId) {
        ...
        try {
            result = await db.query(
                `SELECT id, created, modified, expires, submission_status, questionnaire ->> 'version' AS "questionnaire_version" FROM questionnaire WHERE id = $1 AND questionnaire -> 'answers' -> 'owner' ->> 'owner-id' = $2`,
                [questionnaireId, ownerId]
            );
        } catch (err) {
            throw err;
